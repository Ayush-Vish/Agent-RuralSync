...
import { CalendarIcon, ClockIcon, CreditCardIcon, MailIcon, MapPinIcon, PackageIcon, UserIcon } from "lucide-react"
import { Button } from "@/components/ui/button"
import { useDashboardStore } from "@/stores/booking.store"
import { Input } from "@/components/ui/input"
import { Label } from "@/components/ui/label"
import { markBookingAsPaid } from "@/stores/booking.store"
...
export default function BookingDialog({
  ...
  const handleUpdateExtraTask = async (taskId: string, updatedTask: { description: string; extraPrice: string }) => {
    try {
      ...
      <DialogContent className="sm:max-w-[600px]">
        ...
        {!loading && !error && booking && (
          <div className="grid gap-4 py-4 sm:grid-cols-2">
            ...
            <div className="grid grid-cols-4 items-center gap-4">
              <UserIcon className="h-4 w-4" />
              <span className="col-span-3">{booking.serviceProvider?.name}</span>
            </div>
                ...
                <ul className="space-y-2">
                  {booking.extraTasks.map((task: any, index: number) => (
                    <li key={index} className="flex justify-between items-center">
                      ...
                      <Button onClick={() => handleDeleteExtraTask(task._id)} variant="destructive" size="sm">
                        Delete
                      </Button>
                    </li>
                  ))}
